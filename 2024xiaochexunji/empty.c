...
/* GRP_SLEEP — motor driver nSLEEP pin (PB16).
 * Not generated by SysConfig; defined manually here. */
...
static void motor(int32_t l, int32_t r)
{
    ...
    /* SysConfig生成了两个独立2通道定时器: PWM_LEFT_INST(TIMA1) 和 PWM_RIGHT_INST(TIMA0)
     * 每个定时器: CC0=正转, CC1=反转 */
    if (l >= 0) {
        DL_TimerA_setCaptureCompareValue(PWM_LEFT_INST,  0, DL_TIMER_CC_0_INDEX);
        DL_TimerA_setCaptureCompareValue(PWM_LEFT_INST,  l, DL_TIMER_CC_1_INDEX);
    } else {
        DL_TimerA_setCaptureCompareValue(PWM_LEFT_INST, -l, DL_TIMER_CC_0_INDEX);
    ...
    if (r >= 0) {
        ...
        DL_TimerA_setCaptureCompareValue(PWM_RIGHT_INST,  r, DL_TIMER_CC_1_INDEX);
    } else {
        DL_TimerA_setCaptureCompareValue(PWM_RIGHT_INST, -r, DL_TIMER_CC_0_INDEX);
