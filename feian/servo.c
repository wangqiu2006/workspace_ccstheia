/**
 * @file servo.c
 * @brief 舵机驱动实现 - 整合到 xiaochexunji 项目
 */
...
/**
 * @brief 简单阻塞延时（与项目中的 delay_ms 保持一致）
 * @note  32MHz下约 4000次NOP ≈ 1ms
 */
...
{
    /* SysConfig 已完成 PWM_SERVO 的底层初始化 */
    s_current_angle = 90U;
...
void Servo_SetPulse(uint16_t pulse_counts)
{
    ...
    /*
     * 仅在 SysConfig 中配置了 PWM_SERVO 时才写寄存器。
     * 当前项目使用直流电机（无舵机），此函数为空操作。
     */
